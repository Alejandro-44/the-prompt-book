...
class PromptsRepository:
    ...
    async def get_one(self, prompt_id: PyObjectId) -> Prompt | None:
        pipeline = [
            {
                "$match": {
                    "_id": prompt_id
    ...
    async def update(self, prompt_id: PyObjectId, update_data: dict) -> bool:
        result = await self.__collection.update_one({ "_id": prompt_id }, { "$set": update_data  })
        return result.modified_count > 0

    async def delete(self, prompt_id: PyObjectId) -> bool:
